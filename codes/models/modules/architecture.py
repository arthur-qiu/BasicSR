...
class RRDB_AFLNet(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, upscale=4, norm_type=None, \
            act_type='leakyrelu', mode='CNA', upsample_mode='upconv'):
        super(RRDB_AFLNet, self).__init__()
        n_upscale = int(math.log(upscale, 2))
        if upscale == 3:
            n_upscale = 1

        self.fea_conv = B.conv_block(in_nc, nf, kernel_size=3, norm_type=None, act_type=None)
        rb_blocks1 = [B.RRDB(nf, kernel_size=3, gc=32, stride=1, bias=True, pad_type='zero', \
            norm_type=norm_type, act_type=act_type, mode='CNA') for _ in range(nb//2)]
        rb_blocks2 = [B.RRDB(nf, kernel_size=3, gc=32, stride=1, bias=True, pad_type='zero', \
                             norm_type=norm_type, act_type=act_type, mode='CNA') for _ in range(nb - nb//2)]
        self.LR_conv = B.conv_block(nf, nf, kernel_size=3, norm_type=norm_type, act_type=None, mode=mode)
        ...
        self.model1 = B.sequential(*rb_blocks1)
        self.model2 = B.sequential(*rb_blocks2)
        self.model3 = B.sequential(*upsampler, HR_conv0, HR_conv1)

    def forward(self, x, z = 0, a = 0):
        y = self.fea_conv(x)
        x = self.model1(y)
        x = self.model2(x + a*z)
        x = self.LR_conv(x) + y
...
class BackFeeder(nn.Module):
    def __init__(self, in_nc, base_nf, out_nc):
        super(BackFeeder, self).__init__()

        self.conv1 = nn.Conv2d(in_nc, base_nf, 3, 1, 1)
        self.BN1 = nn.BatchNorm2d(base_nf)
        self.ReLU = nn.ReLU(True)
        self.conv2 = nn.Conv2d(base_nf, out_nc, 3, 1, 1)
